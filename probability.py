...
def probability(paths):
    ...
    ax.set_xticks(index, ['Path 1', 'Path 2', 'Path 3', 'Path 4', 'Path 5'])
    ax.legend(loc='best')

    return fig


def probabilityStandalone(paths):
    # Calculation
    for path in paths:
        # Political sentiment
        pathSentiment = 0
    ...
    for path in paths:
        ...
        xLabel.append(s)
    #fig, ax = plt.subplots()
    plt.bar(index,probabilityList,width, label="Probability", tick_label=xLabel)
    plt.ylabel("Probability")
    plt.title("Probability of Paths")
    plt.xticks(index)
    plt.legend(loc='best')
