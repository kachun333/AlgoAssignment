...
class pathHolder:
    ...
    def __gt__(self, holder2):
        if self.probability == holder2.probability:
            return self.distance > holder2.distance
        else:
            self.probability < holder2.probability
...
if __name__ == '__main__':
    ...
    print('path get ########## Com')
    paths[0].probability = 0.51
    print(paths[0].probability)
    paths[1].probability = 0.60
    paths[2].probability = 0.53
    paths[3].probability = 0.54
    paths[4].probability = 0.55
    print('sort')
    paths.sort(key=getP, reverse=True)
    for path in paths:
        print(str(path.path) + '\t\t\t' + str(path.distance) + '\t\t\t' + str(path.probability))
